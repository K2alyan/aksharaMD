...
**Verifier hardening:** `benchmarks/eval_v1/verify_study_freeze.py` §7 (added in this correction) now reads `TRACK_C_PROMPT_V1.txt`, normalizes its content to LF, computes the canonical-LF SHA-256, and compares it against the pinned value.  A mismatch fails the freeze verifier with a non-zero exit code.  An automated regression test (`tests/test_verify_study_freeze.py`) was added to prevent silent drift.
