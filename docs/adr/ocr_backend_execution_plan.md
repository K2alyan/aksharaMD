...
|---|---|---|---|
| A — Evidence | not started | — | Blocked on: A0 gate (download authorization + disk-space + GPU-time budgets from user). |
...
- **A1 — inspect, hash, commit revision + trusted manifest.** Reviews the A0 report against the upstream HuggingFace file list. Commits:
  - `_UNLIMITED_OCR_MODEL_REVISION` (40-char SHA) in the adapter.
  - A new committed file `benchmarks/pdf_benchmark_adapters/unlimited_ocr_trusted_manifest.json` — machine-generated, versioned via a `manifest_schema_version` field — listing every executable or configuration-sensitive file (every `.py`, every `*_config.json`, `tokenizer.json`, plus any file whose contents are loaded at model init) with SHA-256 and file size.
  - `_UNLIMITED_OCR_TRUSTED_MANIFEST_VERSION` constant so verification code can refuse a snapshot whose manifest schema is older than expected.
  - Removes the inline `_UNLIMITED_OCR_TRUSTED_CODE_FILES` dict in favor of the committed JSON manifest as the single source of truth.
  - New tests: unexpected extra executable rejected, missing file rejected, modified file rejected, symlink/path-traversal rejected, case-sensitivity collision rejected (Windows-visible), partial-download (any file with size < recorded size) rejected.

  **Invariant.** `verify_trusted_code_files()` runs — and must return True — before any `transformers` import or `AutoModel.from_pretrained` call, in *every* code path (benchmark adapter, production port, `aksharamd models verify`). No test may bypass verification with a plain field assignment except by pre-populating a valid trusted manifest under a monkeypatched `HF_HOME`.

- **A1.5 — feasibility smoke test.** Runs Unlimited-OCR on exactly THREE predeclared assets:
...
- **E1 — `models install`.** Operational contract:
  ...
  - **License display.** Shows the model's license (Apache-2.0 for `baidu/Unlimited-OCR`) and requires either an interactive `y` confirmation or `--i-accept-license <license-id>` in non-interactive mode. No download without acceptance.
