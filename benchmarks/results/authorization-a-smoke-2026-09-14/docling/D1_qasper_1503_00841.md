...
This maximum entropy term can be derived by setting the constraint function to G (x , y ) = I ⃗ ( y ) . Therefore, E p θ ( y | x) [ G (x , y )] is just the model distribution p θ ( y | x) and its expectation with the empirical distribution ˜ p (x) is simply the average over input samples, namely p ( y ) . When S takes the maximum entropy form, we can derive the objective function as above.
...
- [Daum´ e III2008] Hal Daum´ e III. 2008. Cross-task knowledge-constrained self training. In Proceedings of the conference on empirical methods in natural language processing , pages 680-688. Association for Computational Linguistics.
...
- [Grac ¸a et al.2007] Joao V Grac ¸a, Kuzman Ganchev, and Ben Taskar. 2007. Expectation maximization and posterior constraints.
