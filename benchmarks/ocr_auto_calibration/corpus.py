...
class CorpusEntry:
    ...
    ``stable_identity`` — when set, this is used as the cache-key identity
    instead of the file's SHA-256. Populated for synthetic fixtures whose
    PDF bytes vary across regenerations (PyMuPDF injects volatile
    creation-date/xref metadata) but whose semantics are stable. The
    actual on-disk SHA is still recorded in ``RunResult.document_sha256``
    and in the acquisition envelope for provenance.
    """
...
def list_synthetic_fixtures(synth_dir: Path | None = None) -> list[CorpusEntry]:
    ...
    Synthetic entries carry a ``stable_identity`` built from the recipe
    hash (sibling ``.hash`` file) prefixed with ``synthetic:v1:``. This
    keeps cache entries stable across PyMuPDF metadata drift while still
    invalidating whenever the recipe (page counts, image content,
    expected backend) actually changes.
    """
    ...
    for pdf_path in sorted(directory.glob("*.pdf")):
        ...
        # Recipe hash is the source of truth for equivalence; fall back to a
        # label-derived digest if the .hash file is missing (shouldn't happen
        # in practice, but keeps the harness resilient to partial fixtures).
        hash_path = pdf_path.with_suffix(".hash")
        if hash_path.exists():
            recipe_hash = hash_path.read_text(encoding="utf-8").strip()
        else:
            recipe_hash = ""
        stable_identity = (
            f"synthetic:v1:{recipe_hash}" if recipe_hash else None
        )
        entries.append(
            CorpusEntry(
