...
At 61.4% on the full 506-doc sample, AksharaMD sits just above PyMuPDF4LLM (60.9%) and below the other open-source local parsers — pypdf (62.5%), MarkItDown (64.5%), and PyMuPDF Text (68.3%) — with commercial tools further ahead (AWS Textract 74.8%, LlamaParse Agentic 89.7%). The full dataset includes many enterprise PDFs with multi-column layouts, headers/footers with legal boilerplate, and OCR-required pages. AksharaMD's noise stripping deliberately trades raw content-faithfulness (which measures the verbatim presence of source sentences, including headers, footers, and boilerplate) for cleaner LLM input — the resulting Markdown is compact and semantically dense even though ParseBench's sentence-presence check penalises the discarded noise.
