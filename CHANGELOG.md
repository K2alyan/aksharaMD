...
- **Unit abbreviations flagged as word-split fragments**: short measurement abbreviations ("pct", "mph", "psi", "cfm", "rpm", "lbs", etc.) were counted by the `short_alpha` fragment check and could cause data tables to be rejected as word-split noise. Added to `_FRAG_WHITELIST`.
